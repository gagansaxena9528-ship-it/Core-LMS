    ...
    <div className={cn("bg-card border border-border rounded-2xl overflow-hidden shadow-sm", className)}>
      {title && (
        <div className="px-6 py-4 border-b border-border flex items-center justify-between bg-card/50">
          <h3 className="font-syne font-bold text-[15px] text-foreground">{title}</h3>
          {headerAction}
        </div>
      )}
...
export const StatCard: React.FC<StatCardProps> = ({ icon, value, label, change, trend, color = 'blue', onClick }) => {
  ...
  return (
    ...
    >
      <div className={cn("text-xl md:text-2xl mb-3 md:mb-4 p-2 md:p-3 rounded-xl bg-muted/5 w-fit transition-transform group-hover:scale-110 duration-300", iconColorMap[color])}>
        {icon}
      </div>
      <div className="font-syne font-extrabold text-2xl md:text-3xl mb-1 text-foreground tracking-tight">{value}</div>
